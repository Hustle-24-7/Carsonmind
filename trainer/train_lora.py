"""
CarsonMind LoRA (Low-Rank Adaptation) 微调脚本  # ！修正：原MiniMind改为CarsonMind
...
# CarsonMind相关组件
from model.CarsonModel import CarsonMindConfig  # 模型配置
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CarsonMind LoRA Fine-tuning")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="CarsonMind-LoRA", help="wandb项目名"
    )
    ...
    # 📚 模型配置初始化
    # CarsonMindConfig: 定义模型的超参数，如隐藏维度、层数、是否使用MoE
    ...
    if args.use_wandb and is_main_process():
        ...
        # 📚 实验名称生成
        # 包含关键参数，便于识别不同的实验配置
        wandb_run_name = f"CarsonMind-LoRA-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
